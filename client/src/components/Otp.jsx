...
function Otp() {
    ...
    const fetchOtp = async () => {
        try {
            let tempUser = await axios.get(`${process.env.REACT_APP_BACK_URL}/user-api/get-tempuser/${email}`);
    ...
    async function addUser(decodedCode) {
        try {
            if (decodedCode === otp) {
                await axios.post(`${process.env.REACT_APP_BACK_URL}/user-api/check-otp`, { otp: decodedCode });
                alert("OTP Verified");
                navigate("/login");
            } else {
                alert("Invalid OTP");
    ...
    const handleResend = async () => {
        ...
        setTimer(90);
        try {
            const response = await axios.put(`${process.env.REACT_APP_BACK_URL}/user-api/resend-otp/${email}`);
